...
export async function postMultipleFiles({
  beforeGeneralPath,
  files,
}: {
  files: File[];
  ...
  try {
    ...
    await Promise.all(
      files.map(async (file) => {
        const document: any = await postToStorageFirebase({
          file,
          path: beforeGeneralPath + file.name,
        });
        //fileResult.data.snapshot, fileResult.data.downloadURL
        if (document.error) return;
        downloadListObjects.push({
          snapshot: document.data.snapshot,
          downloadURL: document.data.downloadURL,
        });
      })
    );
    // wich one will you keep ??
    return { data: { files: downloadListObjects }, error: false };
  } catch (e: any) {
    return { error: true, message: e.message };
  }
}
